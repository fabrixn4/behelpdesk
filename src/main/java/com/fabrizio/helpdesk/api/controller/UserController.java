...
public class UserController {
	...
	public ResponseEntity<Response<User>> create(HttpServletRequest request, @RequestBody User user
		...
		try {
			...
			User u = (User) userService.createOrUpdate(user);
			response.setData(u);
		} catch (DuplicateKeyException d) {
			response.getErrors().add("Mail duplicated!");
			return ResponseEntity.badRequest().body(response);
		} catch (Exception e) {
	...
	private void validateCreateUser(User user, BindingResult bindingResult) {
		if (user.getEmail() == null) {
			bindingResult.addError(new ObjectError("User", "Email not informed"));
		}
	}
	...
	private void validateUpdateUser(User user, BindingResult bindingResult) {
		if (user.getId() == null) {
			bindingResult.addError(new ObjectError("User", "Id not informed"));
		}
		if (user.getEmail() == null) {
			bindingResult.addError(new ObjectError("User", "Email not informed"));
		}
